...
时光倒流，让我们回到那个熟悉的时刻——幼儿园时光。还记得你学到的第一件事吗？没错，字母表。当你掌握了所有26个字母后，阅读的大门就为你敞开。作为Solana开发者，这正是你学习之旅的起点。阅读英语的能力使你成为了一个能够理解和消化知识的超级大脑。

现在，是时候再次采取行动了。我们将从你幼儿园老师应该教授的基础开始——从区块链中读取数据。
...
你可以通过调用 JSON RPC 的API来获取所需的内容，它会与网络沟通并返回你所需的数据。
...
这是一个简单的Next.js应用程序，安装了所有依赖项后，你可以使用 `npm run dev` 命令启动它。你应该能在`localhost上`看到如下内容：
