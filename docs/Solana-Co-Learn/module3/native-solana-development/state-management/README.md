...
Solana保持其速度和效率的一部分原因是程序是无状态的。这意味着你无法更改程序上的数据 - 所有数据都存储在外部账户中，通常是由程序拥有的账户。大多数这些账户是PDA（Program Derived Accounts） - 我们现在将查看它们的数据存储元素，并稍后深入了解其余部分。
...
现在要求免租金，所以只有选项#2。这种方法的最大优点是，当你不再需要存储数据时，可以销毁账户并取回你的SOL！区块链上的免费存储（减去交易费用）🥳
...
当你不需要签署交易时，使用 `invoke` 。当你需要签署交易时，使用 `invoke_signed` 。在我们的情况下，我们是唯一可以为PDA签署的人，因此我们将使用 `invoke_signed` 。
...
更新帐户数据的第一步是将其数据字节数组反序列化为Rust类型。你可以通过首先借用帐户上的数据字段来实现此操作。这样可以在不获取所有权的情况下访问数据。


然后，你可以使用`try_from_slice_unchecked`函数来反序列化借用账户的数据字段，使用你创建的类型来表示数据的格式。这将为你提供一个Rust类型的实例，以便你可以使用点符号轻松更新字段。如果我们使用我们一直在使用的笔记应用程序示例来完成这个操作，它会是这样的：
...
一旦Rust实例更新了账户数据的适当值，你可以将数据“保存”在账户上。

这是通过对你创建的Rust类型实例使用`serialize`函数来完成的。你需要传入对账户数据的可变引用。这里的语法有些棘手，所以如果你不完全理解也不用担心。借用和引用是Rust中最难的概念之一。
...
更新 `programId` 中的 `index.ts` 为你程序的地址，然后运行 `npm run start` 。
