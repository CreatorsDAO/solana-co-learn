...
我们已经从指令中提取了数据，计算出了需要支付的租金，现在我们需要一个账户来存储它。嗨，PDAs！还记得前端的鸡和蛋问题吗？这里也是一样的情况！我们将从程序ID和一组种子中推导出账户地址。
...
CPI可以使用 `invoke` 或 `invoke_signed` 来实现。
...
这通过对你创建的Rust类型实例使用`serialize`函数完成。你需要传入账户数据的可变引用。这里的语法有些复杂，所以如果你不完全理解也不必担心。借用和引用是Rust中最复杂的概念之一。
...
就是这样，朋友们。你现在了解了如何在Solana上向账户写入数据。我真的很激动，因为这太重要了。给你一个巨大的赞扬，拿着它，你值得！
