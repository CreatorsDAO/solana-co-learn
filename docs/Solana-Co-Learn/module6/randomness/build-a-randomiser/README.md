...
在`InitUser`上下文中有四个账户。状态是我们的用户状态对象，其中包含`vrf`和`payer`密钥种子，这是用于测试的版本。对于生产代码，您只需要`payer`种子。我们这样做是为了节省时间，而不是使用环境变量。然后有`vrf`账户，switchboard不会自动加载它，因此需要使用.load()调用来加载。可能有其他使用switchboard的方法，但我们目前使用的是最简单/最快的路径来启动和运行，随时可以探索和改进它。最后，我们有`payer`和`system`程序来创建一个新账户。
