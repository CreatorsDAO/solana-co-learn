...
const sharedConnectors = new Map<SerializedOptions, WalletConnectLegacyConnector | WalletConnectConnector>();
...
function createConnector(
  ...
  const connector =
    version === '1' ? new WalletConnectLegacyConnector(config as any) : new WalletConnectConnector(config as any);
...
export function getWalletConnectConnector({
  ...
  return sharedConnector ?? createConnector(version, config as any);
}
